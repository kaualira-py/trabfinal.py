...
def dieta():
    ...
    rot = str(input("Você pode fazer exercícios físicos diariamente ? (S/N): "))
    if p == 1:
        print("""Dieta para GANHO DE PESO:
...
- Total do dia: 2755 calorias""")
        ...
        print("""Dieta para PERCA DE PESO RAPIDAMENTE 
...
""")
        ...
        print("""Dieta para MANTER O PESO: 
...
def exercicio():
    pergunta = input("Você quer uma lista de exercícios ? (S/N)".upper())
    if pergunta == "S":
        exercicios = ["Agachamento", "Flexão", "Corrida", "Abdominal", "Pular corda", "Prancha"]
        horas = float(input("Quantas horas você pode dedicar aos exercícios ? "))
        min_totais = horas * 60
